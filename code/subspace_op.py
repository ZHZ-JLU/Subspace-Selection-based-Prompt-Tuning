...
parser.add_argument("--intrinsic_dim", default=2, type=int)
...
parser.add_argument("--subspace_learn_num", default=10, type=int)
parser.add_argument("--subspace_learn_dim", default=1000, type=int)
parser.add_argument("--subspace_learn_budget", default=8000, type=int)
parser.add_argument("--idx_list", nargs='+', type=int, default=[2,7])
...
A_save_path = f"subspace_learned/A_learned_{model_name.replace('/', '-')}_{task_name}_{n_prompt_tokens}_{seed}_{args.subspace_learn_num}_{args.subspace_learn_dim}_{args.subspace_learn_budget}.pt"
...
def mcp_prox_operator(delta, lr, Lambda, beta):
    mask1 = abs(delta) > beta * Lambda
    prox_result1 = torch.where(mask1, delta, 0.0)

    mask2 = (abs(delta) > lr * Lambda) * (abs(delta) <= beta * Lambda)
    prox_tmp = torch.sign(delta)*((abs(delta)-lr*Lambda) / (1-lr/beta))
    prox_result2 = torch.where(mask2, prox_tmp, 0.0)

    prox_result = prox_result1 + prox_result2
...
class LMForwardAPI:
    def __init__(self, model_name='roberta-large', n_prompt_tokens=50, task_name='sst2',
        ...
        if random_proj == 'normal':
            ...
            self.A = torch.load(A_save_path).T.to(device)[:,idx_list]
            
        self.best_train_perf = 0.0
        self.best_dev_perf = 10.0
        self.best_num_call = 0
    ...
    def my_dev_eval(self, prompt_embedding=None):
        if self.num_call % self.eval_every == 0:
            ...
            ##if dev_perf > self.best_dev_perf:
                ...
                ##self.best_num_call = self.num_call
            ...
            print('Dev loss: {}. Dev perf: {}. Best dev perf: {}. Best num call: {}.'.format(
                round(float(dev_loss), 4),
                round(float(dev_perf), 4),
                round(float(self.best_dev_perf), 4), self.best_num_call))
            print('********* Done *********')
...
while model_forward_api.num_call < budget:
    for mini_batch_train_data in all_mini_batch:
        ...
        #z_t = mcp_prox_operator(z_t_before_prox, eta, Lambda, beta)
        z_t = z_t_before_prox
