...
parser.add_argument("--intrinsic_dim", default=1000, type=int)
...
parser.add_argument("--parallel", default=False, help='Whether to allow parallel evaluation')
...
parser.add_argument("--learn_dim", default=10, type=int)
...
learn_dim = args.learn_dim
A_save_path = f"subspace_learned/A_learned_{model_name.replace('/', '-')}_{task_name}_{n_prompt_tokens}_{seed}_{learn_dim}_{intrinsic_dim}_{budget}.pt"
Acc_save_path = f"subspace_learned/A_learned_{model_name.replace('/', '-')}_{task_name}_{n_prompt_tokens}_{seed}_{learn_dim}_{intrinsic_dim}_{budget}.txt"
...
class LMForwardAPI:
    def __init__(self, model_name='roberta-large', n_prompt_tokens=50, task_name='sst2',
                 loss_type='hinge', init_prompt_path=None, A=None, config=None, tokenizer=None, model=None):
        self.config=config,
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        if isinstance(prompt_embedding, list):  # multiple queries
            ...
            for pe in prompt_embedding:
                ...
                #z = self.linear(z)  # Az
                z = torch.matmul(self.A, z.to(device))
            ...
            prompt_embedding = torch.matmul(self.A, prompt_embedding.to(device))
            if self.init_prompt is not None:
...
if model_name in ['roberta-base', 'roberta-large']:
    ...
    config = CPTConfig.from_pretrained(model_name)
    tokenizer = BertTokenizer.from_pretrained(model_name)
    model = CPTForMaskedLM.from_pretrained(
        model_name,
        config=config,
        n_prompt_tokens=n_prompt_tokens,
    )
...
if random_proj == 'normal':
    ...
    if model_name in ['roberta-base', 'roberta-large']:
        embedding = model.roberta.get_input_embeddings().weight.clone().cpu()
    elif model_name in ['bert-base-uncased', 'bert-large-uncased']:
        embedding = model.bert.get_input_embeddings().weight.clone().cpu()
    elif model_name in ['google/electra-base-generator', 'google/electra-large-generator']:
        embedding = model.electra.get_input_embeddings().weight.clone().cpu()
    elif model_name in ['facebook/bart-base', 'facebook/bart-large', 'fnlp/cpt-large']:
        embedding = model.model.get_input_embeddings().weight.clone().cpu()
    elif model_name in ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl']:
        embedding = model.transformer.get_input_embeddings().weight.clone().cpu()
    else:  # T5
        embedding = model.get_input_embeddings().weight.clone().cpu()
...
z = torch.zeros(learn_dim, n_prompt_tokens*config.hidden_size).to(device)
A = torch.normal(mean=mu, std=std, size=(n_prompt_tokens*config.hidden_size, intrinsic_dim)).to(device)
...
for i in range(args.learn_dim):
    print('Learing {}-th subspace'.format(i+1))
    ...
    best_prompt = torch.tensor(model_forward_api.best_prompt).type(torch.float32).to(device)
    z[i] = torch.matmul(A, best_prompt)

    B = torch.normal(mean=mu, std=std, size=(n_prompt_tokens*config.hidden_size, intrinsic_dim)).to(device)
    #B = torch.randn(n_prompt_tokens*config.hidden_size, intrinsic_dim).to(device)
    C = torch.nn.functional.normalize(z[:i+1], dim=1).T
    #tmp = torch.matmul(C.T, C)

    A = B - C @ C.T @ B
    
    #tmp2 = torch.matmul(z[:i+1], A)
    '''
    tmp = torch.concat((z[:i+1], torch.zeros(intrinsic_dim, n_prompt_tokens*config.hidden_size)), dim=0).to(device)
    U, S, V = torch.svd(tmp)
    A = V[:,i+1:]
    #tmp2 = torch.matmul(tmp, A)
    '''

torch.save(z, A_save_path)
np.savetxt(Acc_save_path, np.array(test_accs))